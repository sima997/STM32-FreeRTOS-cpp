...
void SystemInit(void)
{
  ...
  SCB->VTOR = VECT_TAB_BASE_ADDRESS | VECT_TAB_OFFSET; /* Vector Table Relocation in Internal SRAM */
    ...
    //3. Turn on FLASH prefetch buffer
    FLASH->ACR |= (1 << FLASH_ACR_PRFTEN_Pos);

    //4. Set PLL input clock to HSI
    RCC->PLLCFGR |= RCC_PLLCFGR_PLLSRC_HSI;
    //5. Configure PLL
    RCC->PLLCFGR &= ~RCC_PLLCFGR_PLLN_Msk; //Clear N
    RCC->PLLCFGR |= (32 << RCC_PLLCFGR_PLLN_Pos); //Set N = 32
    RCC->PLLCFGR &= ~RCC_PLLCFGR_PLLM_Msk; //Clear M
    RCC->PLLCFGR |= (1 << RCC_PLLCFGR_PLLM_Pos); //Set M = 4 = (3+1) -> 1
    RCC->PLLCFGR &= ~RCC_PLLCFGR_PLLR_Msk; //Clear R
    RCC->PLLCFGR |= (0 << RCC_PLLCFGR_PLLR_Pos);//Set R = 2
    RCC->PLLCFGR |= RCC_PLLCFGR_PLLREN; //Enable R output
    RCC->PLLCFGR &= ~RCC_PLLCFGR_PLLPDIV_Msk; //Clear P
    RCC->PLLCFGR |= (4 << RCC_PLLCFGR_PLLPDIV_Pos); //Set PLL P = 4
    RCC->PLLCFGR |= RCC_PLLCFGR_PLLPEN;
    //6.Turn PLL on
    RCC->CR |= RCC_CR_PLLON;
    //7.Set PLL as system source
...
/*
    ...
    // 2. Configure PLL: PLLCLK = (HSI / PLLM) * PLLN / PLLR
    RCC->PLLCFGR = RCC_PLLCFGR_PLLSRC_HSI   // PLL source = HSI
                 | (1 << RCC_PLLCFGR_PLLM_Pos)   // PLLM = 1
                 | (8 << RCC_PLLCFGR_PLLN_Pos)   // PLLN = 8
                 | (0 << RCC_PLLCFGR_PLLP_Pos)   // PLLP unused
                 | (0 << RCC_PLLCFGR_PLLQ_Pos)   // PLLQ unused
                 | (0 << RCC_PLLCFGR_PLLR_Pos);  // PLLR = 2 (0b00)
